 ...
 */
export default function WhatsAppButton({ product, phone, message, variant = 'primary', className = '' }) {
  ...
  const base = 'inline-flex items-center justify-center gap-2 font-bold transition-all cursor-pointer hover:scale-105 active:scale-95';
  const shape = variant === 'floating' ? '' : 'h-12 px-6 rounded-xl';

  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className={`${base} ${shape} ${styles[variant]} ${className}`}>
      {variant === 'floating' ? (
